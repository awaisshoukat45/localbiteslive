...
<form id="order-form" role="form" method="post" action="{{route('order.store')}}" autocomplete="off"
    ...
    <div class="row">
            ...
            <div class="form-group{{ $errors->has('addressID') ? ' has-danger' : '' }}">
                @if(count(auth()->user()->addresses))
                    ...
                    <select name="addressID" id="addressID"
                            class="form-control{{ $errors->has('addressID') ? ' is-invalid' : '' }}" required>
        ...
        <div class="col-md-2">
            <div class="form-group">
                ...
                <button type="button" data-toggle="modal" data-target="#modal-order-new-address"
    ...
    <div class="form-group{{ $errors->has('phone') ? ' has-danger' : '' }}">
        ...
        <input name="phone" id="phone" type="text" class="form-control{{ $errors->has('phone') ? ' is-invalid' : '' }}"
               placeholder="{{ __( 'Your phone here' ) }} ..." value="{{auth()->user()->phone}}" required>
        @if ($errors->has('phone'))
            <span class="invalid-feedback" role="alert">
    ...
    <div class="form-group{{ $errors->has('comment') ? ' has-danger' : '' }}">
        ...
        <textarea name="comment" id="comment" class="form-control{{ $errors->has('comment') ? ' is-invalid' : '' }}"
                  placeholder="{{ __( 'Your comment here' ) }} ..." required></textarea>
    ...
    <div class="form-group">
        ...
        @if (!env('HIDE_COD',false))
            <div class="custom-control custom-radio mb-3">
                <input name="paymentType" class="custom-control-input" id="cashOnDelivery" type="radio"
                       value="cod" {{ env('DEFAULT_PAYMENT','cod')=="cod"?"checked":""}}>
        ...
        @if (env('STRIPE_KEY',false)&&env('ENABLE_STRIPE',false))
            <div class="custom-control custom-radio mb-3">
                <input name="paymentType" class="custom-control-input" id="paymentStripe" type="radio"
                       value="stripe" {{ env('DEFAULT_PAYMENT','cod')=="stripe"?"checked":""}}>
        ...
        @if (env('STRIPE_KEY',false)&&env('ENABLE_STRIPE_IDEAL',false))
            <div class="custom-control custom-radio mb-3">
                <input name="paymentType" class="custom-control-input" id="paymentIdeal" type="radio"
                       value="ideal" {{ env('DEFAULT_PAYMENT','cod')=="ideal"?"checked":""}}>
        ...
        @if(count(auth()->user()->addresses)&&!env('HIDE_COD',false))
            <div class="text-center" id="totalSubmitCOD"
                 style="display: {{ env('DEFAULT_PAYMENT','cod')=="cod"&&!env('HIDE_COD',false)?"block":"none"}};">
...
@if (env('STRIPE_KEY',false)&&env('ENABLE_STRIPE',false))
    <form action="/charge" method="post" id="stripe-payment-form"
          style="display: {{ env('DEFAULT_PAYMENT','cod')=="stripe"?"block":"none"}};">
        ...
        <div style="width: 30em" class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
            <input name="name" id="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}"
                   placeholder="{{ __( 'Name on card' ) }}" value="{{auth()->user()->name}}" required>
            @if ($errors->has('name'))
                <span class="invalid-feedback" role="alert">
...
@if (env('STRIPE_KEY',false)&&env('ENABLE_STRIPE_IDEAL',true))
    <form id="ideal-payment-form" style="display: {{ env('DEFAULT_PAYMENT','cod')=="ideal"?"block":"none"}};">
        ...
        <div style="width: 30em" class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
            <input name="name" id="name-ideal" type="text"
