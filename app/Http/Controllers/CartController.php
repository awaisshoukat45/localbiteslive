...
class CartController extends Controller
{
       ...
       // Cart::clear();
        if($item && $canAdd){
            ...
            foreach ($request->extras as $key => $value) {
                 if($item->extras()->findOrFail($value)->extrasGroup->name == "Deselection"){
            ...
            foreach ($request->selections as $key => $value) {
                   $cartItemName.="\n ".$item->subItems()->findOrFail($value)->name;
            }
            Cart::add((new \DateTime())->getTimestamp(), $cartItemName, $cartItemPrice, $request->quantity, array('id'=>$item->id, 'extras'=>$request->extras,'selections'=>$request->selections,'comment'=>$request->comment,'restorant_id'=>$restID,'image'=>$item->icon,'friendly_price'=>  Money($cartItemPrice, env('CASHIER_CURRENCY','usd'),true)->format() ));
   ...
   */
    ...
    public function cart(){
        $restorantID=null;
        /*Bilal:It check how many items are including in cart so on that basices it find restaurant ids*/
        foreach (Cart::getContent() as $key => $cartItem) {
            $restorantID=$cartItem->attributes->restorant_id;
            break;
        }
        /*Bilal: Now the help of that restorant ids we need to find restaurant information*/
        //The restaurant
        $restaurant=Restorant::findOrFail($restorantID);

        /*Bilal: Now He define all time slotes for use in checkout procedure*/
        //Create all the time slots
        $timeSlots=$restaurant->hours?$this->getTimieSlots($restaurant->hours->toArray()):[];
